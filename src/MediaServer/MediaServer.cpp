...
BaseSource * MediaServer::loadMedia(string & path, int mediaType, ofLoopType loopType){
	...
	if(mediaType == SourceType::SOURCE_TYPE_IMAGE){
		...
		return loadVideo(path, loopType);
...
BaseSource * MediaServer::loadVideo(string & path, ofLoopType loopType){
	...
	if(isVideoLoaded){
		...
		// Notify objects registered to onImageLoaded event
		stringstream ss;
		ss << "Video " << path << " already loaded, restart video";
        videoSource->restartVideo(path);
		ofLogNotice("MediaServer") << ss.str();
	...
	videoSource->loadVideo(path, loopType);
